...
with st.expander("ℹ️ Как рассчитывается каждый параметр"):
    simple_explanation = pd.DataFrame([
        {
            "Параметр": "Тип почвы",
            "Простыми словами": "Насколько ваша почва подходит для конкретной культуры.",
            "Пример": "Для пшеницы чернозём — идеальный вариант (балл максимальный). Подзолистая почва для той же пшеницы почти не подходит (балл около нуля).",
        },
        {
            "Параметр": "pH почвы",
            "Простыми словами": "Насколько кислотность вашей почвы попадает в комфортный для культуры диапазон.",
            "Пример": "Если культура любит pH 6–7, а у вас 6.5 — балл максимальный. Если у вас pH 4.5 — балл заметно ниже.",
        },
        {
            "Параметр": "Вегетационное окно",
            "Простыми словами": "Достаточно ли тёплых дней между весенними и осенними заморозками, чтобы культура успела вырасти и созреть.",
            "Пример": "Культуре нужно 90 дней роста. Если в зоне 140 безморозных дней — хватает с запасом (балл высокий). Если всего 80 — не успеет созреть (балл низкий).",
        },
        {
            "Параметр": "Тепловая сумма (GDD)",
            "Простыми словами": "Хватает ли зоне накопленного за сезон тепла, чтобы культура не просто выросла, а дала урожай.",
            "Пример": "Теплолюбивому подсолнечнику в холодной северной зоне тепла не хватит (балл низкий). В тёплой южной степи — хватит с запасом (балл высокий).",
        },
        {
            "Параметр": "Влагообеспеченность",
            "Простыми словами": "Хватает ли влаги культуре — и по общему количеству осадков, и по риску долгих засушливых периодов подряд.",
            "Пример": "Влаголюбивый рапс в засушливой зоне получит низкий балл. Засухоустойчивые подсолнечник или чечевица в той же зоне — высокий.",
        },
        {
            "Параметр": "Жаростойкость",
            "Простыми словами": "Выдержит ли культура типичное для зоны количество по-настоящему жарких дней (выше 30°C).",
            "Пример": "В прохладной северной зоне жарких дней почти нет — параметр не критичен ни для кого. В жаркой степи культуры с низкой жаростойкостью (например овёс) получат низкий балл.",
        },
        {
            "Параметр": "Урожайность",
            "Простыми словами": "Справочный показатель потенциальной урожайности культуры (сколько центнеров с гектара она обычно даёт). От условий вашего поля НЕ зависит.",
            "Пример": "Ячмень обычно даёт больше центнеров с гектара, чем чечевица — поэтому у ячменя балл по этому параметру выше, независимо от почвы или климата.",
        },
    ])
    render_wrapped_table(simple_explanation, col_widths_pct=[16, 34, 50])
